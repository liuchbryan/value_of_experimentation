...
def get_prioritisation_value_samples(
    ...
    sigma_sq_X, sigma_sq_1, sigma_sq_2, nu,
    verbose=True):
    ...
    for i in range(0, n_samples):
        # Step 1
        propositions = (
            pd.DataFrame(mu_X + np.random.standard_t(nu, N) * sigma_X, 
                         columns=['true']))

        # Step 2
        propositions['observed_noisy'] = (
            propositions['true'] + 
            mu_epsilon + np.random.standard_t(nu, N) * sigma_1)
        ...
        propositions['observed_clean'] = (
            propositions['true'] + 
            mu_epsilon + np.random.standard_t(nu, N) * sigma_2)
